...
import { loadPatientNotes } from '../services/PatientNotesApi';
import type { PatientNote } from '../types/PatientNotesTypes';
import { collectPatientNoteTags, groupPatientNotes } from '../utils/PatientNotesGrouping';
import PatientMoodLegend from './PatientMoodLegend';
import PatientNoteCard from './PatientNoteCard';

const allTagsValue = 'all';

export default function PatientNotesWorkspace() {
  ...
  const availableTags = useMemo(() => collectPatientNoteTags(notes), [notes]);
  const filteredNotes = useMemo(
    () => notes.filter((note) => selectedTag === allTagsValue || note.activityTags.includes(selectedTag)),
    [notes, selectedTag],
  );
  const groupedNotes = useMemo(() => groupPatientNotes(filteredNotes), [filteredNotes]);
