...
def build_kafka_consumer(
    ...
    """Create a Kafka consumer with the runtime topic and group settings."""
...
def run_kafka_consumer_loop(
    ...
    while not shutdown_event.is_set():
        ...
        try:
            consumer = build_kafka_consumer(settings, broker_list)
            LOGGER.info(
                "Kafka consumer subscribed topic=%s brokers=%s",
                settings.kafka_chat_topic,
                broker_list,
            )
            ...
            LOGGER.exception("Kafka consumer connection failed; retrying.")
        except Exception:
            LOGGER.exception("Kafka consumer crashed unexpectedly; retrying.")
        finally:
